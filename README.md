...
- **失败回滚**:变更采用"暂存副本 + 落盘 + 提交"串行执行,落盘成功后才提交到内存;落盘失败时暂存副本直接丢弃,内存记录、幂等键与序号都不受影响,文件保持原内容;同一幂等键可安全重试,失败记录不会被后续写入意外持久化。
- **读取可见性**:读取接口只能看到已成功落盘的状态;写入等待落盘期间,未提交的记录对读取不可见,不会出现先出现再消失的记录。
