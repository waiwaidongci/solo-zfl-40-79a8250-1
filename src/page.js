...
<body>
  ...
  <script>
    ...
    async function load() {
      ...
      var results = await Promise.all([api("/api/items?" + params.toString()), api("/api/batches"), api("/api/stats")]);
      items = results[0]; batches = results[1]; stats = results[2];
      render();
    }
